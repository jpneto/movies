...
This table includes movies I think have a higher than average quality in either plot, main themes and overall cinematography. This list represents `r 100 * round(nrow(df) / nrow(df.imdb),3)`% of [all movies I've seen](https://www.imdb.com/user/ur3443219/ratings?ref_=nv_usr_rt_4) and is, obviously, quite subjective :-)
...
barplot(mat %>% t, names.arg=unique(df3$Decade), ylim=c(0,375), col=brewer.pal(6, "Blues") )
legend(.25, 350, rev(c('7+','7.5+','8+','8.5+','9+','9.5+')), 
...
text(y=plt, x=df6$n+9, label=df6$n, pos=2, cex=0.8, col='blue')
```
...
text(y=plt, x=df2$n+5.5, label=df2$n, pos = 2, cex = 0.8, col='blue')
```
